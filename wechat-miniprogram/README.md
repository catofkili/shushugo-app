...
未关联过的微信身份不会在普通登录请求中自动创建账号。设置页和组队页会让用户明确选择新建，或在「我的」输入已有收集日账号邮箱、用一次性邮件验证码把当前微信关联到旧账号；App / Apple 账号也可通过登记邮箱完成这一步。若该微信身份已经关联到另一收集日账号，Worker 会返回冲突，不自动搬动身份、学习数据或权益。此实现还需要 Worker 配好 `RESEND_API_KEY` 才能发送关联验证码；源码接通不代表线上凭据、邮件送达或真机链路已验收。部署时需先更新 Worker，再紧接着上传新小程序；旧客户端缺少显式新建标记，新 Worker 会暂时拒绝它们首次登录的新微信身份。
