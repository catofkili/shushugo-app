// 微信的 require 只认 .js，不认 .json。src/data/*.json 是源（其中 kanji_orthography.json
// 要和 iOS 端逐字节一致），这里给每份生成同名 .js 包一层 module.exports；
...
if (process.argv[1] && path.resolve(process.argv[1]) === new URL(import.meta.url).pathname) {
  if (fs.existsSync(manualReviewPath)) {
    fs.writeFileSync(path.join(dir, 'confusion_manual_review.json'), JSON.stringify(await extractManualReview(), null, 2) + '\n');
  }
  for (const file of fs.readdirSync(dir).filter((name) => name.endsWith('.json'))) {
    fs.writeFileSync(path.join(dir, file.replace(/\.json$/, '.js')), moduleSource(fs.readFileSync(path.join(dir, file), 'utf8')));
