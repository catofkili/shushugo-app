...
### 2026-09-24 期限卡状态更新（覆盖上文“客户端只售永久版”）

用户本轮报告月、季、年、永久四档商品都已填写。代码现按以下映射接单：
`pro_monthly`（¥10 / 1 个北京时间自然月）、`pro_quarterly`（¥24 / 3 个月）、
`pro_yearly`（¥68 / 12 个月）、`pro_lifetime`（¥298 / 永久）。前三项是普通道具的一次性购买，
到期后手动续购，不会自动扣费。微信道具 ID 与内部商品 ID、Worker 的分价必须逐项一致。

本次只更新了本地源代码与价格配置；没有部署 Worker、没有上传小程序，也没有独立读取后台各道具的发布状态。
线上健康接口当前仍报告 `wechatPayConfigured=true`、`wechatPushConfigured=true`、`migrationsApplied=true`、
`productionReady=false`。因此四档代码接线不等于可售或提审完成；还要确认四档在目标环境已发布，核对沙箱 AppKey，
完成真机取消/支付/补查/退款，再分别核对提审门槛与正式环境。
