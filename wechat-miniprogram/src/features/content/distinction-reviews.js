...
// ../frontend/src/data/confusion_distinction_reviews.ts
var confusion_distinction_reviews_exports = {};
__export(confusion_distinction_reviews_exports, {
  DISTINCTION_REVIEWS: () => DISTINCTION_REVIEWS,
  distinctionNotesFor: () => distinctionNotesFor,
  distinctionReviewFor: () => distinctionReviewFor,
  distinctionReviewMap: () => distinctionReviewMap
});
module.exports = __toCommonJS(confusion_distinction_reviews_exports);
var DISTINCTION_REVIEWS = [
  {
...
var SYNONYM_REVIEWS = SYNONYM_REVIEW_ROWS.map(
  ([groupKey, level, summary]) => ({ groupKey, level, summary })
);
var distinctionReviewMap = new Map(
  [...DISTINCTION_REVIEWS, ...SYNONYM_REVIEWS].map((review) => [review.groupKey, review])
);
var distinctionReviewFor = (groupKey) => distinctionReviewMap.get(groupKey) ?? null;
var plainForm = (text) => text.replace(/\[[^\]]+\]/g, "").replace(/\s+/g, "").trim();
var plainReviewText = (text) => text.replace(/\[[^\]]+\]/g, "").trim();
var escapeRegExp = (text) => text.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
var distinctionNotesFor = (summary, targets) => {
  ...
  if (!cleanSummary || !allForms.length) return /* @__PURE__ */ new Map();
