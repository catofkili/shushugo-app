...
起点水平、固定新词额度、试用到期后仅单词等后续决策见 [起点水平备考计划](LEVEL_PLAN_DEV_PLAN.md) 第 5 节；这一节保留当时圆环与模式的设计过程。
...
## 6. 手写汉字配对题与辨析卡准入补充（2026-09-24）

汉字卡正面改为配对：左边是含目标字的词，右边是打乱的候选读音；用户逐项连线，翻面显示词的假名和正确连线。题源是 `frontend/scripts/kanji-reading-questions-manual.json`，每组由人选词、读音和必要的排除理由；构建器只负责核对种子词库与汉字单元索引，不自动造题。候选按一一对应分配：同一读音不能连给两个词，选中的读音会从原词移开。当前 520 个多音字组、1,258 组词/读音配对全部通过构建校验。凡没有可出题的手写配对，或同一词面可能对应多个读音的情况，都不进入这类 FSRS 清单；既有单字记忆与复习流水保留。

辨析 Anki 卡只从人工审校为 `major` 的非同义组产生。`interchangeable`、`synonym`、同音异义与同字异读组不进队列；卡面先回想整组词义差别，翻面才看手写辨析，不用情景猜选项。选择题入口另要求组员题面首义互不重复，避免多个成员成为同一答案。来源仍是单一文件 `frontend/src/data/confusion_distinction_reviews.ts`。

汉字连线要逐项查看并操作，Web 混合学习把这张卡的“无交互后停止累计学习时长”阈值设为 120 秒；普通单词和其他卡继续使用 `study-clock.ts` 的 60 秒默认值。切卡时先结算上一张卡的有效时间，页面切到后台仍立即暂停，回来后的空白时段不会补记。横屏矮视口让题头收紧、词表在卡内滚动，底部显示答案和评分键保持可见。
