...
由于 iOS 不允许一个 App 读取另一个 App 的私有数据库，收集日不直接读取 MOJi 的 `.realm` 文件。可复现的方式是在已登录 MOJi 的 Mac 上生成一个中立 JSON，再通过 AirDrop 或“文件”导入 iPhone。
...
5. 脚本将在桌面生成 `shushugo-moji-review-export.json`。它只包含词条、读音、释义和复习状态；不会包含 MOJi 登录凭据。
6. 将此 JSON 通过 AirDrop 发送到 iPhone，在收集日的“我的 → 设置 → 导入词单或 MOJi 复习记录”中选择它。
