...
const LOAD_STICKER = { light: "scene-stretch", steady: "scene-book", heavy: "mood-fired-up", aggressive: "mood-dizzy" } as const;
...
export function LoadCurve() {
  ...
  return <div className="ds-card mb-4 p-5">
    <div className="flex items-baseline justify-between gap-3">
      <div><p className="ds-kicker">未来 8 周预计压力</p><h3 className="mt-1 text-xl font-black jp-ink">{TIER[load.tier]}</h3></div>
      <span className="ds-pill ds-num">峰值约 {peak.minutes} 分钟/天</span>
    </div>
    <div className="mt-5 flex h-32 items-end gap-2" aria-label="未来八周每日预计分钟">
      {load.perWeek.map((week) => <div key={week.week} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-1">
        <small className="ds-num text-[11px] font-bold jp-muted">{week.week === load.peakWeek ? "最重" : week.minutes}</small>
        ...
        <small className="text-[11px] jp-muted">{week.week}周</small>
      </div>)}
    </div>
    {/* 这句解释以前是一行灰色小字；现在让吉祥物按压力大小换个表情说 */}
    <MascotSay sticker={LOAD_STICKER[load.tier]} size={48} tone={load.tier === "aggressive" ? "warn" : "info"} className="mt-4">
      第 {load.peakWeek} 周最重，每天大约 <b>{peak.minutes} 分钟</b>。这是按标准速度估的，你自己的节奏可能快、也可能慢。
    </MascotSay>
  </div>;
}
