...
const minutesFor = (plan: Plan, active: PlanKind[]) => Math.round(active.reduce((sum, kind) => sum + (plan[kind].fresh + plan[kind].review) * SECONDS_PER_CARD[kind], 0) / 60);
...
export const DailyPlanPanel = ({ compact = false }: Props) => {
  ...
  const active: PlanKind[] = entitlements.isPro ? PLAN_KINDS : ["words"];
