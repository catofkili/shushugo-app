 ...
 *
 * **单位是「张卡」不是「个词」**：一个词的正向 / 反向 / 汉字读音是三张各自调度的卡
 * （见 word-api/directions.ts），语法点也是一张。问「今天要学多少」的时候，
 * 答的就是这个数 —— 按词去重会把同一天背了正向又背了反向说成一次。
 *
...
/** 未来预测要看的四张卡表。汉字/反向/语法平时数量很小，但它们也是要还的账。 */
const FORECAST_ENTITIES: FsrsEntity[] = [WORD_FSRS, REVERSE_FSRS, KANJI_FSRS, GRAMMAR_FSRS];
...
const pastCounts = (firstDay: string, lastDay: string) => {
  ...
  // 一张卡 = (词, 方向)。同一天同一张卡答十次也只算一张 —— 那十次是学习步骤在
  // 反复确认同一件事，不是学了十个东西。
  rowsFor(`
    SELECT reviewed_on, COUNT(DISTINCT word_id || '/' || direction) AS cards
    FROM reviews
  ...
  rowsFor(`
    ...
    FROM (
      SELECT word_id, direction, MIN(reviewed_on) AS reviewed_on
      FROM reviews
  ...
  rowsFor(`
    SELECT reviewed_on, COUNT(DISTINCT grammar_id) AS cards
  ...
  rowsFor(`
    SELECT reviewed_on, COUNT(*) AS fresh
    FROM (
      SELECT grammar_id, MIN(reviewed_on) AS reviewed_on
      FROM grammar_reviews
      GROUP BY grammar_id
    )
    WHERE reviewed_on BETWEEN ? AND ?
...
const forecastDue = (dayCount: number, now: Date): number[] => {
  ...
  FORECAST_ENTITIES.forEach((entity) => {
    ensureFsrsColumns(entity);
    rowsFor(`
      SELECT fsrs_due FROM ${entity.table}
      WHERE ${entity.eligible} AND fsrs_due >= ? AND fsrs_due < ?
    `, [from, to]).forEach((row) => {
...
const pendingDue = (now: Date): number => {
  ...
  return FORECAST_ENTITIES.reduce((sum, entity) => {
    ensureFsrsColumns(entity);
    return sum + Number(rowsFor(`
      SELECT COUNT(*) AS n FROM ${entity.table}
