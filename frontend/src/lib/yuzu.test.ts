...
import { buyItem, equippedItem, ownsItem, repairableDays, repairDay, repairPrice, settleYuzu, yuzuBalance, YUZU } from "./yuzu";
...
describe("收入", () => {
  ...
  it("清完计划 = study + plan = 10;连击第 7 天 +30;成就每个 20,补发的老成就也给", () => {
    answers(120);
...
describe("消费", () => {
  ...
  it("补签:只补 7 天内、首次打卡之后的洞;30 天内 50/100/200 递增;补完连击接上", () => {
    ...
    fund(150);
    expect(repairPrice()).toBe(50);
    expect(repairDay("2026-09-18")).toBe(true);
    expect(repairPrice()).toBe(100);
    expect(repairDay("2026-09-16")).toBe(true);
    expect(repairPrice()).toBe(200);
