 ...
 *
 * 每日新学数从 studyPreferences 读（kanjiDailyGoal / confusionDailyGoal），复习不设上限：
 * 到期的全出。汉字的候选按备考目标等级过滤。
 */
...
import {
  ...
  materializeKanjiChars, pickKanjiCharNext, recordKanjiCharReview, undoLastKanjiCharReview, type KanjiCharCard
} from "./kanji-char-cards";
import {
  confusionCardProgress, createConfusionTasks, matchingCard, materializeConfusionCards,
  pickConfusionNext, recordConfusionReview, undoLastConfusionReview, type MatchingCard
...
export const getKanjiCardSession = (db: object, day = today()): KanjiCardSession => {
  materializeOnce(db);
  createKanjiCharTasks({ fresh: getStudyPreferences().kanjiDailyGoal, review: UNLIMITED }, targetLevelRank(), day);
...
export const getConfusionCardSession = (db: object, day = today()): ConfusionCardSession => {
  materializeOnce(db);
  createConfusionTasks({ fresh: getStudyPreferences().confusionDailyGoal, review: UNLIMITED }, day);
