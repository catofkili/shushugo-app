...
import { hasKanjiText } from "../models/word-card";

/**
 * 一个词 = 三张卡:正向(释义 → 日语)、反向(日语 → 释义)、汉字(释义 → 汉字)。
 *
 ...
 */
export type DirectionId = "forward" | "reverse" | "kanji";
...
export const KANJI: StudyDirection = {
  ...
  phase: "kanji",
  entity: KANJI_FSRS,
  taskTable: "kanji_progress",
  // 汉字卡只对「写法和读音不同」的词有意义;更细的判断(真的含汉字而不是符号)
  // 交给 hasKanjiText,SQL 这道只是粗筛
...
/**
 * 泛化代码要读的非 FSRS 计数列。kanji_memory 是种子库里带来的老表,
...
export const ensureDirectionCards = (direction: StudyDirection, limit: number): number => {
  if (direction.id === "forward" || limit <= 0) return 0;
  ...
  for (const row of rows) {
    ...
    // 汉字卡的细筛:SQL 只能比较 kanji <> kana,真正有没有汉字要看字符
    if (direction.id === "kanji" && !hasKanjiText(String(row.kanji ?? ""))) continue;
    ...
    if (forward) {
      writeFsrsState(wordId, {
        ...forward,
        stability: Math.max(forward.stability * SEEDED_STABILITY_RATIO, 1),
        due: now,
...
export const directionUnseenCount = (direction: StudyDirection): number => {
  if (direction.id === "forward") return 0;
  ensureDirectionColumns(direction);
  return firstValue<number>(`
    ...
    WHERE p.seen_count > 0
      ...
      ${direction.wordFilterSql ? `AND ${direction.wordFilterSql}` : ""}
      AND NOT EXISTS (
        SELECT 1 FROM ${direction.entity.table} m
        WHERE m.word_id = p.word_id AND COALESCE(m.seen_count, 0) > 0
      )
  `, [], 0);
};
