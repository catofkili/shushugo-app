...
## ⚠️ 小程序的功能层从网页源码打包，不许再手抄一份（2026-09-22 定的）
...
**还没按网页合并的（下一步，按这个顺序）**：① 出厂内容数据包——题面层 `question_meaning_overrides`
（1 MB）、`confusion_distinction_reviews`（336 KB）、`word_sense_keys`（707 KB）放进一个分包，
`require.async` 灌进 shim 的 store；② 有了它才能把 `confusion-groups`（网页 1,933 组 / 小程序自己算
1,881 组）、`distinction-quiz`（小程序现在拿 `words.meaning` 当题面，违反「缺人工题面整组跳过」）、
`models/word-card`（正向题题面）搬进共享层；③ 单词调度（`core/study-core.js` 的 createTodayPlan vs
网页的 stage1 + scheduler：优先级、排片、减负、压轴、顽固词闸都不一样）；④ 混合学习的汉字卡 / 连线卡。
③ 换掉的是小程序核心答题路径，动之前先和用户确认。
