...
- ⚠️ **改成只数正向之后，「今天」那根仍然不等于大卡上的数**（实测 442 vs 320）。
  ...
  每天只放 `LEECH_DAILY_INTAKE`(10) 个进去（2026-09-18 起只在上限装得下时生效）。**这条差额是故意留着的**，理由同下面
...
### ⚠️ 上限装不下时选词是纯随机，不按任何推词逻辑（2026-09-18，用户定的）

`fsrsDueWordIds`：到期词数 > 复习上限 → `ORDER BY RANDOM()`，**没有「刚栽跟头优先」、
没有 due 顺序、也没有顽固词 10 个的闸**。装得下时全进，老排序只影响 `order_index`。
...
闸本身不减少顽固词，只是把它们藏起来。上限装不下时它已经不生效；装得下时还留着，
要不要整个删是用户的决定，还没定。
