...
（2026-09-26：`stem:済`（済ます / 済ませる）改为 `interchangeable`，二者只是新旧形，Codex 稿里写的「済ませる 可让别人办完」是错的；现在是 344 组。）

⚠️ **手写稿的格式「共同判据；词：提示；词：提示」由 `distinctionNotesFor` 按「词：」标签拆**（2026-09-26 修）。
Codex 把 367 组改成这个格式后，按旧的「句子里包含哪个词形就挂给谁」拆出三类静默错误：
判据首段点名成员（「事件「終わる」，任务「終える」」）→ 整条要点又抄进两张成员卡；
`たどり着く：…` 包含 `着く` → 挂到 着く 卡上；提示内部再用「；」续写的无名短句 → 直接丢掉（14 组）。
现在只要后续分段里出现「成员词形：」，首段就只当判断要点，分段按标签精确认领，无标签短句归前一个词。
旧写法（「甲是…；乙是…」）仍走老逻辑。⚠️ **判断要点内部不许用「；」**——卡片按第一个分号截取要点。
另：这批稿子 345 组里 326 组的要点是「看 A，还是 B」、69 组仍写着「自己 / 主动 / 自然」，
真正能在句子里看的线索（が / を / に、后项）多在成员提示里；要点本身有待逐组再磨（见 `docs/CONFUSION_POINT_WRITING_STANDARD.md`）。
...
- JLPT 每日新词最低量和起点预估都用目标范围内当前真正未学的词（`seen_count = 0` 且 `known_forever = 0`）除以剩余可进新天数，并随作答或批量标认识逐次重算。起点只决定哪些等级按先验熟悉处理，不能用 N4→N3 等固定等级差词数覆盖用户在目标范围内已有的真实学习记录。
...
- JLPT 考试次日首次打开首页时，显示一次「辛苦了，考试结束啦」提示，可调整下一场目标、采用轻量计划或保持原强度。未选择原强度时，考后 7 天暂停自动新增单词/语法/汉字/辨析，也不提供自动续杯加餐；单词复习每天最多 60、语法 10、汉字 10、辨析 5。保存的平时额度不改，第 8 天自动恢复。提示选择按考试日期记住，调整同一场的目标不会重复提示；明确选择原强度会立刻恢复平时额度。该规则只跟已设的 JLPT 考期走，不套用到 EJU 等考试。旧 JLPT 计划进入 past 后不再催考试覆盖量或显示复习欠账。
...
### 2026-09-25 首月赠送会员（仅微信小程序）

- Pro 仍是付费会员。小程序预计上线后的首个月内，每个账号可领一次 30 天赠送会员；窗口关闭后不再发放，因此最晚领取者的赠送期约在预计上线两个月后结束。网页和原生 App 不发放此赠送。`LAUNCH_GIFT_CLAIM_UNTIL` 在提审时按预计上线日加一个月填写；窗口必须在审核期间保持开启，否则审核员无法走通会员功能。部署关闭窗口前先核对真实上线时间。
- 暂时隐藏现有 7 天完整计划试用：首次建计划不再请求领取，试用接口返回 `TRIAL_DISABLED`。赠送资格单独写入 `launch_gift_grants`，不复用 `trial_grants`，因为旧表每账号的主键会挡住已经领过 7 天试用的用户领取首月赠送；重试必须读回原到期日，不能续送。
- Taro/W4 登录接好后，在登录成功时调用 `claimLaunchGift()`；会员功能提示按钮只在平台为 `wechat` 且 Worker 返回 `launchGift.open` 时显示「登录领取首月会员」，并由调用方把 `onRequireAuth` 接到登录流程。当前 `App.tsx` 的「7 天试用结束啦」弹窗留待 W2 合入 `main` 后再改，接手时一起删除旧 7 天试用到期提醒逻辑。
...
- ~~还没做：分享长图仍是 V3 的样子~~ —— 2026-09-24 两套都画了（`5f140e3`），见下一节。
  小程序的周报页仍没跟着改（它是另一套 WXML）。
