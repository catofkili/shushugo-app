  ...
  return (
    <>
      ...
      {position && createPortal(
        <span
          className="pointer-events-none fixed z-[2147483647] w-72 rounded-md border border-white/15 bg-[#202424] p-3 text-left text-xs leading-5 text-white shadow-2xl"
    ...
    <>
      {parts.map((part, index) => {
        ...
        return hint ? (
          ...
          <span key={`${part}-${index}`}><JapaneseRuby text={part} /></span>
        );
      })}
    </>
  );
